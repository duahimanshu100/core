...
from .models import SmAccount, ProfileMetric , PostLatestMetric
...
class PostLatestMetricSerializer(serializers.ModelSerializer):
    '''
        Serializer for PostLatestMetric model
    '''
...
class PostWithLatestMetricSerializer(serializers.ModelSerializer):
    '''
        Serializer for PostLatestMetric model
    '''
...
class PostsListWithVisionSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        post_metric = PostLatestMetric.objects.filter(
            post_id=instance).first()
        post_hash_tags = PostHashTag.objects.filter(
            post_id=instance)
        post_filters = PostFilter.objects.filter(
            post_id=instance)
        if post_vision:
            data['post_vision'] = PostsVisionSerializer(post_vision[0]).data
        if post_metric:
            data['post_metric'] = PostLatestMetricSerializer(post_metric).data
        if post_hash_tags:
            data['post_hash_tags'] = PostHashTagSerializer(
                post_hash_tags, many=True).data
        if post_filters:
